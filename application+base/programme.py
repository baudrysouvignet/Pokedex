...
def AffichezTypePokemon():
    
    sqliteConnection = connexion()
    cursor = sqliteConnection.cursor()
    #ecriture de la requéte
    sqlite_select_Query = "SELECT idPokemon,nom,HP,libelle_type FROM pokemon INNER JOIN type  ON type.idType = pokemon.idType WHERE libelle_type LIKE '%" + var_texte_recherche.get() + "%';"
    #execution de la requéte
    cursor.execute(sqlite_select_Query)
    #on place tout les enregistrements dans une variable record
    record = cursor.fetchall()
...
#bouton de recherche
bouton_affichez_pokemon=Button(fenetre, text="Rechercher-nom", command=AffichezListePokemon)
bouton_affichez_pokemon.place(x=170,y=450,width=150, height=20)

bouton_affichez_pokemon_type=Button(fenetre, text="Rechercher-type", command=AffichezTypePokemon)
bouton_affichez_pokemon_type.place(x=350,y=450,width=150, height=20)
